...
const { tokens, external } = require('./statemachine.js');
...
const dexprices = (dex_list) => {
  ...
  return Promise.all(dexes.map(async (dex) => {
    ...
    wavax = find_token(tokens, { key: 'symbol', value: 'wavax'} );
    eth = find_token(tokens, { key: 'symbol', value: 'eth'} );
    wbtc = find_token(tokens, { key: 'symbol', value: 'wbtc'} );
